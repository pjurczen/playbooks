...
|------------------------------------------------------------------------------------------------------------------------------------------|---------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `ProduktRecalculationService` (+ `ProduktRecalculationServiceTest`)                                                                      | new     | one responsibility: product recalculation. `recalculate(Angebot, ProduktCalculationContext)` calls the gateway and reconciles — no lookups, no flush. `recalculateAngebote(List<AngebotNr>, Bearbeiter)` orchestrates split → parallel compute → finalise — no calculation inline. Batch logic lifted from `ProduktCalculationCollectorService.calculateDirtyAngebote` |
