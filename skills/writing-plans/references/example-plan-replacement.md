...
|------------------------------------------------------------------------------------------------------------------------------------------|---------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `ProduktRecalculationService` (+ `ProduktRecalculationServiceTest`)                                                                      | new     | one responsibility: product recalculation. `recalculate(Angebot, ProduktCalculationContext)` — orchestration: gateway call, then the offer reconciles itself; no lookups, no flush, no mapping. `recalculateAngebote(List<AngebotNr>, Bearbeiter)` — orchestration: split → parallel compute → finalise; no calculation inline. Batch logic lifted from `ProduktCalculationCollectorService.calculateDirtyAngebote` |
...
|---|------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------|---------------------------------------------------------------|
| 1 | Service beside the collector | both operations, unit-tested; nothing wired; the design's dependency rule (callers never reach the gateway) added to the architecture test | `ProduktRecalculationServiceTest` and the architecture test green; reactor compiles | none — additive |
