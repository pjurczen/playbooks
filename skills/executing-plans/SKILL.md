...
These mean *the design is wrong*, not *this milestone is hard*. **STOP — do not keep patching.** Surface what you've learned to the user and go back to the design (re-open brainstorming for the affected seam — for a slice, possibly the initiative's seam; a `proposed` ADR is revised there, in place, never duplicated). Ten accreted patches shipped as "done" is the failure this catches, and the tremors are visible long before the end-of-feature review — which is far too late to unwind a wrong abstraction.
