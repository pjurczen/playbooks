...
v0.4.0's consolidation model routed "a decision + its rationale" into an ADR at landing. In practice that produced implementation summaries: by landing time the agent's context is the diff, the review and the findings block, and every instruction to stay at design altitude was fighting that gravity — the ADRs came out sixty lines long, full of class names and annotations, with a findings list for context and no alternatives, one per decision per feature. Three things were wrong at once: the *moment* (the decision is made in brainstorming when the user picks an approach; recording it weeks later records what was built), the *bar* (no notion of "architecturally significant", so every routed item became an ADR), and the *owner* (no skill defined an ADR's shape).

v0.7.0 moves capture to the decision point and gives it an owner. `writing-adr` is the only skill that writes ADR text; `brainstorming` invokes it after design approval when the chosen approach clears a conjunctive three-part bar (constrains future work, hard to reverse, there was a real choice), and the ADR is `proposed` on the feature branch until `consolidating-docs` reconciles and promotes it on landing. No code exists at brainstorming time, so altitude is enforced structurally rather than by rule. Most features produce no ADR — that is the intended default, not a gap.

Two smaller choices follow from this. **Date-prefixed filenames** are the bootstrap default (never numbers): a `proposed` ADR can sit on a branch for the whole feature, and two in-flight features would race for the same number. **The design doc gained a required "approach chosen and why" section** (one line plus a link when an ADR exists): it is the input to consolidation's fallback path and to the drift reconcile, and the most durable thing in an otherwise ephemeral document. Everything repo-specific — where ADRs live, how they're named, which language, whether there's an index — stays in `.claude/documentation.md`; the skill hardcodes none of it.

---

## 12. Addendum — Design docs and plans: two readers, two layers (2026-09)

§4.1 argued that plans should describe intent, not implementation, because Opus would be the implementer and a fully specified plan is a worse version of what it would write itself. That premise has shifted: in practice the design and plan are handed together to an implementer in another session — Opus-class by default, occasionally a cheaper model when the user judges the work simple — or to a teammate, to implement — and they are read and presented in team meetings first. So the artifacts have two readers with opposite needs: one wants to follow the idea without the codebase, the other wants nothing left to guess. Real designs were serving neither: investigation reports followed by 60–300 lines of Java bodies, no alternatives on disk, plans that restated the design and added milestones, an umbrella design patched with divergence callouts until it was false.

The resolution is layering, not cutting. A design reads top-down: Problem, Approach (with the alternatives it beat), Shape (one mermaid diagram, the components) as the narrative a teammate follows in two or three minutes; then Contracts and numbered Guarantees as the precision layer. The altitude rule is **contracts, not bodies**: a public signature with its semantics belongs in a design — it is what the implementer must match and a reviewer can check — while a body belongs to the implementer and rots before the plan is written (one real design declared a method deletable; its plan found three callers). The plan is the precision layer's second half and is now *more* exact than §5.4 sketched: a changes table by class and method, call-site from → to, scenarios tied to Guarantees by number, milestones naming suites. It never restates the design. §5.0's "readable in 2–3 minutes" now applies to the narrative layer; the pair is as long as completeness needs, and the test is the implementer, not the clock.

Two smaller decisions follow. **Mermaid** is the diagram notation for user-facing artifacts because the team's viewers (GitHub/GitLab/Bitbucket, IntelliJ) render it natively; the skills' own `dot` graphs are for the agent and never appear in a design. **Initiative designs** (`docs/playbooks/initiatives/`) exist, optionally, for work that won't fit one cycle and whose slices share a target architecture — the category the 600-line umbrella was, without a home. They hold the shared shape and contracts and a slices-with-status table, are edited rather than annotated, and are consolidated only when the last slice lands.

---

## 13. Addendum — field check of the artifact shapes (2026-09)

Before dogfooding §12, the shapes were checked against the field: Google's design-doc anatomy and the SWE book, Oxide RFDs, Rust RFCs, PEPs, HashiCorp's RFC culture, Amazon narratives, arc42, C4 and Diátaxis on the human side; GitHub Spec Kit and Kiro (templates fetched verbatim), superpowers' writing-plans and subagent skills, PRP, Anthropic's Claude Code guidance and AGENTS.md studies on the agent side; and the Ambig-SWE, Agentless and SWE-bench Verified papers on how models cope with under-specified tasks.

Every structural choice held. The design/plan split is universal. Contracts-not-bodies is Google's "rarely contain code" and Spec Kit's quickstart rule, and Agentless found a signature skeleton beats full files as model input. The two layers are Rust's guide-level/reference-level and Diátaxis' explanation/reference. "Name the question before the diagram" is Google's caption-first rule. Numbered, cited guarantees are Kiro's requirement lines and Spec Kit's identifiers. The changes and call-site tables are the best-evidenced element of all: file and function localisation is the measured hard step, and Ambig-SWE recorded a six-point gain for a strong model just from being told file locations.

What was missing and is now added: `Status`, `Assumptions` and `Open questions` (every RFC culture has the last), "Done means", failure behaviour in contracts, a preservation guarantee, a milestone table for the meeting, red-first stated in the plan (a superpowers issue documents implementers in a separate session skipping it otherwise), a binding *Stop and ask if* list (the two-readers essay's "highest-leverage line"; Ambig-SWE shows models don't ask unless told), two-way coverage in the self-review, a `Deviation:` rule so a design stays true during execution, and a sizing carve-out for changes describable in one sentence. Alternatives and guarantees were loosened to what reviewers actually need.

What was deliberately not taken, because it is the superpowers direction — completeness for the cheapest implementer at the cost of every human reader: signatures on every plan row, a restated constraints block, read-first precedent lists, pseudo-code in designs, a cross-cutting checklist as content. The target implementer is Opus-class; the plan names what such an implementer would otherwise have to search for, and the stop-and-ask list covers the occasional cheaper run. Two honest caveats from the sources: no credible evidence shows agents implement better from diagrams (the diagram serves the meeting; bullets and contracts carry the load), and superpowers' own guidance puts a mid-tier floor under prose plans, which is exactly where this design sits.

---

## 14. Addendum — lean skills, and evals as the release bar (2026-09)

§10 reversed the original plan and put Red Flags tables on nearly every skill because "practice showed they drive compliance cheaply". That was judgment, not measurement, and an audit of the library against published guidance turned it over. Anthropic's prompting guide for the current model family says skills written for prior models are often too prescriptive and that the "CRITICAL: you MUST" framing built to fight under-triggering now over-triggers; the instruction-count research shows adherence falling with every added rule, earlier rules winning; superpowers' own `writing-skills` scopes rationalization tables to discipline skills and requires each row to come from an excuse observed in a baseline run. The library at v0.7.0 was 15,200 words, its bootstrap 1,118 words loaded every session, and its rules were restated three to five times each.
...
The v0.7.0 design shape had been derived from a single worked example and its rules carried that example's accidents — a before/after diagram, twelve nodes, javadoc signatures — as if they were the shape. The sections were the right concerns; the rules under them were not general, and a word budget I had set for skill files pushed me to compress them rather than generalise. The correction follows Anthropic's own guidance for judgment tasks: the process stays lean in `SKILL.md`, and breadth lives in references read when relevant — a kinds-of-work table read at the first step, a view catalogue read per view, one worked example read per kind, an initiative reference read only when decomposition fires.

Three rules changed substance rather than form. Design-level decisions get their own section, because most decisions a design makes are below the ADR bar and had no home once the ADR was scoped correctly. The mechanism may appear in a design when it is the decision — an algorithm whose complexity is the point, a state machine, a concurrency scheme — under the test of whether an implementer given only the contracts would build something materially different; the earlier "no code" rule had targeted pasted bodies and hit the one case where code is the design. And an initiative is a design plus a roadmap, produced by brainstorming and by a new `writing-roadmap` skill: the design owns the target architecture, the coexistence mechanism and the shared contracts; the roadmap owns the slices and their order and is revised as slices land. Slicing is planning, not design, and a mode that changes the artifact is a skill, not a bullet.

A second correction came from reading the result: the checklist was still a list of activities — where to look, what to write — with no account of how one gets from a request to a design, and it asked for the kind of work to be named before anything was known. The skill is now five reasoning stages, each with one output the next depends on: problem (the need behind the request, with an observable "done"), diagnosis (the cause, checkable), decision (approaches as answers to the diagnosis; the kind of work and the feature-versus-initiative question fall out here), shape, check. The design doc is the trace of that reasoning, with Diagnosis as its own section; the kinds table serves stage four, where it says which views and which contracts, instead of being a classification made up front.

---

## 17. Addendum — structure is gated, not prompted (2026-09)

The user's observation that implementer models mix abstraction levels and couple modules even with contracts in hand turned out to be the best-measured fact in the literature on generated code, and the ranking of remedies was unambiguous: a mechanical gate inside the loop beats any amount of prompting, a fresh-context critic with named findings beats self-review, and decomposition fixed up front beats both prompt engineering and after-the-fact refactoring, which the models cannot reliably do either. The pipeline had the third and none of the first two; its boy-scout pass asked the model that had just written the long method to notice it, in the same context.

Three design choices follow. Gates are repo-owned and discovered, never assumed: `.claude/gates.md` has the same posture as the documentation map, proposed when absent and honest about what is not enforced, and existing violations are a baseline that blocks nothing, because a gate that fails on day one is a gate that gets disabled. The critic is a subagent because the evidence is specifically about context: separation from the generator is the mechanism, so a checklist in the same session would not do. And the reviewer is narrowed rather than strengthened, because the same evidence puts open-ended structural judgment by a model at the bottom, and a reviewer hunting for structural gaps is the failure Anthropic's own harness work warns pushes toward over-engineering. No skill body gained a rule about clean code; the rules went into the critic's prompt and the gate configuration, where instruction-count decay cannot reach them.
