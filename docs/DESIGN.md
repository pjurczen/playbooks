...
§4.1 argued that plans should describe intent, not implementation, because Opus would be the implementer and a fully specified plan is a worse version of what it would write itself. That premise has shifted: in practice the design and plan are handed together to a smaller model, or to a teammate, to implement — and they are read and presented in team meetings first. So the artifacts have two readers with opposite needs: one wants to follow the idea without the codebase, the other wants nothing left to guess. Real designs were serving neither: investigation reports followed by 60–300 lines of Java bodies, no alternatives on disk, plans that restated the design and added milestones, an umbrella design patched with divergence callouts until it was false.
