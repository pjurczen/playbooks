...
Implementer models mix abstraction levels, concentrate responsibility and couple modules even when given contracts. The research says this is general and measured — twenty models maintaining repositories all lost to humans on maintainability while most beat them on lint; real AI commits have functions half again as long and four times the follow-up edits; smell density tracks the volume generated — and it ranks the fixes: mechanical structural gates in the loop first (a static-analysis feedback loop cut maintainability issues about tenfold at no correctness cost), a separate critic fed with named findings second (87% fixed against 34% for "improve this"), decomposition fixed before the implementer starts third; an open-ended LLM reviewer is the weakest lever for structure. Sources: SWE-CI (arxiv 2603.03823), the 12.7k-commit study (2603.27130), the OO-design study (2605.19901), SmellBench (2606.05574), the static-analysis loop (2508.14419), the RL quality-reward paper (2506.02211), IFScale (2507.11538), OpenAI's harness engineering post, Kent Beck's "Augmented coding and design".

The pipeline now carries all three. A per-repo **gates map**, `.claude/gates.md`, names the commands that prove correctness and structure — tests, lint, format, complexity and length, dependency direction, duplication — and what is not enforced; a new **`setting-up-gates`** skill proposes one when it's missing, from a per-stack reference of the lightest tools that cover the three mechanically checkable properties, never imposing and never fixing existing violations, which are recorded as a baseline. `using-git-worktrees` runs the gates as the clean baseline; `executing-plans` treats a milestone as green only when tests and gates both pass; `verifying-before-done` counts a structure claim as proven only by gate output. Designs gain **structural rules** in Contracts — dependency direction, ownership, limits — that milestone one of the plan wires into the config. A **structure critic** subagent runs once per milestone in fresh context, with the diff, the contracts and the gate output, against a fixed six-item checklist, and returns findings by location with the move; the refactor pass applies them. The clean-code checklist left the skill body for the critic's prompt. The end-of-feature reviewer is narrowed to correctness and conformance, and for structure runs the gates instead of judging. Plans carry one clause of **altitude** per new method and one responsibility per new class. And this repo eats its own cooking: `scripts/check-skills.sh` is its structural gate and `.claude/gates.md` its map.
...
`executing-plans`' refactor pass is scoped by what you touched, not by a time-box: code you touched, or that your change made worse or exposed, is cleaned with whatever effort that needs, including restructuring within the touched unit; a bug found there is fixed in the milestone with a scenario; the only deferrals are code you did not touch and contract changes others depend on, which are a stop-and-ask. Cleanliness debt on touched code never becomes a followup, and the reviewer's Followup tier is scoped the same way. The plan carries the design's folded-in improvements as Changes rows, so the cleanup the work owes is planned. Slice mode flips the roadmap row to `in progress` on the base branch when the slice design is approved.
